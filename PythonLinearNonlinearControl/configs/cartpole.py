...
class CartPoleConfigModule():
    ...
    def state_cost_fn(x, g_x):
        ...
        if len(x.shape) > 2:
            # print("g_x in cartpole1: ", g_x[:, :, 0])

            return (6. * (( x[:, :, 0] - g_x[:, :, 0] )**2)
                    + 12. * ((np.cos(x[:, :, 2]) + 1.)**2)
                    + 0.1 * (x[:, :, 1]**2)
                    + 0.1 * (x[:, :, 3]**2))[:, :, np.newaxis]

        elif len(x.shape) > 1:
            # print("g_x in cartpole2: ", g_x[:, 0])
    ...
    def terminal_state_cost_fn(terminal_x, terminal_g_x):
        ...
        if len(terminal_x.shape) > 1:
            return (6. * (( terminal_x[:, 0] - terminal_g_x[:, 0] )**2)
